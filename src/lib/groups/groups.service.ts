...
export class GroupsService {
    ...
    public async joinGroupPublic(yourId: string, groupId: string): Promise<Group> {
        try {
            ...
            if (group.privacy === Privacy.Private) {
                throw new BadRequestException('Liên hệ với admin của group để được thêm vào')
            }
            return await this.addMember(group.admin_id, {
                groupId: group._id,
                userId: yourId,
            }, true)
        } catch (err) {
